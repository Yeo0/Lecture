...
public class Tree {
	...
	private List list;
	private List subList; //아래에 임의로 만든건데 사용하려면 위에서 이렇게 지정해줘야해
	private int height;
	...
	*/
	...
	public Object remove(TreeNode t){ // 그냥 받는 인수// treenode v : 지우려는 애 
		if(!hasLeft(t)&&!hasRight(t)){ //!함수 :(같지않음)//hasleft도 false고 hasright도 false 인 경우 = 양쪽이 비어있는경우
									   //양쪽이 비어있을때 지울 수 있으니까
			if (t.getParent().getLeftChild()==t){//부모의 왼쪽이랑 자식(지우려는애)이 같으면
				t.getParent().setLeft(null); // 데이터를 직접 지우는게 아님,부모와 v의 연결을 끊음. 
											 // data/left/right/parent 에서 right 값을 null로 즉 data칸은 살아있음
				return t.getData(); 
